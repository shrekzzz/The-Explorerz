...
import { uploadImage, deleteImage, uploadDocument } from '../services/upload.service.js';
...
// ─── PPT/Document Upload Configuration ───
...
const documentFilter = (_req: Request, file: Express.Multer.File, cb: multer.FileFilterCallback) => {
  ...
  if (allowedMimes.includes(file.mimetype)) {
    ...
    cb(new BadRequestError(`Invalid file type: ${file.mimetype}. Allowed: PPT, PPTX, PDF`));
  }
};
...
export async function uploadPPT(req: Request, res: Response, next: NextFunction) {
  try {
    if (!req.file) throw new BadRequestError('No file uploaded');
