...
const ALLOWED_DOC_FORMATS = ['pdf', 'ppt', 'pptx'];
...
/**
 * Upload a document (PPT, PDF) to Cloudinary.
 */
export async function uploadDocument(
  ...
  if (fileBuffer.length > MAX_DOC_SIZE) {
    throw new BadRequestError(`File size exceeds ${MAX_DOC_SIZE / 1024 / 1024}MB limit`);
  }
  ...
  try {
    ...
    logger.info({ publicId: result.public_id, bytes: result.bytes }, 'Document uploaded to Cloudinary');
    ...
    logger.error({ err, originalName }, 'Cloudinary document upload failed');
    throw new BadRequestError(err.message || 'Failed to upload document');
  }
}
