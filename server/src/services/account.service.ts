...
async function safeRedisSetex(key: string, ttl: number, value: string): Promise<void> {
  try {
    ...
    logger.warn({ err, key }, 'Redis unavailable, setex skipped');
  }
}
...
async function safeRedisGet(key: string): Promise<string | null> {
  try {
    ...
    logger.warn({ err, key }, 'Redis unavailable, get skipped');
    return null;
  }
}
...
async function safeRedisIncr(key: string): Promise<number> {
  try {
    return await redis.incr(key);
  } catch (err) {
    logger.warn({ err, key }, 'Redis unavailable, incr skipped');
    return 0;
  }
}
...
async function safeRedisExpire(key: string, seconds: number): Promise<void> {
  try {
    ...
    logger.warn({ err, key }, 'Redis unavailable, expire skipped');
  }
}
...
async function safeRedisTtl(key: string): Promise<number> {
  try {
    ...
    logger.warn({ err, key }, 'Redis unavailable, ttl skipped');
    return 0;
  }
}
...
async function safeRedisKeys(pattern: string): Promise<string[]> {
  try {
    ...
    logger.warn({ err, pattern }, 'Redis unavailable, keys skipped');
    return [];
  }
}
...
export async function sendVerificationEmail(userId: string, email: string, firstName: string): Promise<void> {
  ...
  await safeRedisSetex(`verify:${token}`, 86400, userId); // 24h TTL
...
export async function verifyEmail(token: string): Promise<{ email: string }> {
  const userId = await safeRedisGet(`verify:${token}`);
  if (!userId) {
  ...
  // Delete token
  await safeRedisDel(`verify:${token}`);
...
export async function initiatePasswordReset(email: string): Promise<void> {
  ...
  // Store in Redis with 1hr TTL (optional)
  await safeRedisSetex(`reset:${token}`, 3600, user.id);
...
export async function resetPassword(token: string, newPassword: string): Promise<void> {
  const userId = await safeRedisGet(`reset:${token}`);
  if (!userId) {
  ...
  await safeRedisDel(`reset:${token}`);

  // Clear session cache
  const sessionKeys = await safeRedisKeys(`sessions:${userId}:*`);
  if (sessionKeys.length > 0) {
    try {
      await redis.del(...sessionKeys);
    } catch (err) {
      logger.warn({ err, sessionKeys }, 'Redis unavailable, session key cleanup skipped');
    }
  }
...
export async function recordFailedLogin(email: string): Promise<boolean> {
  ...
  if (count === 1) {
    ...
    await safeRedisExpire(key, LOCKOUT_DURATION);
  }
...
export async function isAccountLocked(email: string): Promise<boolean> {
  const key = `lockout:${email.toLowerCase()}`;
  const count = await safeRedisGet(key);
...
export async function clearFailedLogins(email: string): Promise<void> {
  await safeRedisDel(`lockout:${email.toLowerCase()}`);
}
...
export async function getLockoutTTL(email: string): Promise<number> {
  return await safeRedisTtl(`lockout:${email.toLowerCase()}`);
}
